...
const STORAGE_KEY='alhera_school_result_manager_v1';
const BACKUP_VERSION=10;
const OPS_KEY='alhera_school_operations_v1';
...
function normalizeDB(raw){
  ...
  CLASSES.forEach(c=>{
    ...
    d.students=d.students.filter(s=>s&&s.id&&typeof s.name==='string'&&s.name.trim()).map(s=>({id:String(s.id),name:s.name.trim(),parentName:String(s.parentName||s.fatherName||s.guardianName||'').trim(),mobile:String(s.mobile||s.phone||'').trim(),address:String(s.address||'').trim(),photo:typeof s.photo==='string'?s.photo:'',studentCode:String(s.studentCode||s.studentId||s.admissionId||'').trim()}));
...
function openModal(html,afterOpen){closeModal(false);$('modalCard').innerHTML=html;$('modalRoot').classList.remove('hidden');$('modalRoot').setAttribute('aria-hidden','false');document.body.style.overflow='hidden';document.body.classList.add('modal-open');$('modalCard').querySelectorAll('[data-modal-close]').forEach(el=>{el.onclick=()=>closeModal()});if(typeof afterOpen==='function')afterOpen($('modalCard'))}
function closeModal(runCleanup=true){if(runCleanup&&state.modalCleanup)try{state.modalCleanup()}catch(_){ }state.modalCleanup=null;const root=$('modalRoot'),card=$('modalCard');if(!root||!card)return;root.classList.add('hidden');root.setAttribute('aria-hidden','true');card.innerHTML='';document.body.style.overflow='';document.body.classList.remove('modal-open');if(typeof restorePageScroll==='function')restorePageScroll()}
...
function openStudentModal(id=''){
  ...
  <div class="modal-actions"><button class="btn outline" type="button" data-modal-close>Cancel</button><button class="btn primary" type="submit">${id?'Save Changes':'Add Student'}</button></div></form>`,card=>{
    ...
    card.querySelector('#studentForm').onsubmit=async ev=>{ev.preventDefault();const details={studentCode:card.querySelector('#modalStudentCode').value,parentName:card.querySelector('#modalParentName').value,mobile:card.querySelector('#modalStudentMobile').value,address:card.querySelector('#modalStudentAddress').value,photo:currentPhoto};const ok=id?await editStudent(id,card.querySelector('#modalStudentName').value,details):addStudent(card.querySelector('#modalStudentName').value,details);if(ok!==false)closeModal()};
...
function saveOps(ops,message='Saved'){
  try{
    // Keep fee records lightweight: student photos already live in the student record.
    // This prevents repeated base64 photo copies from exhausting mobile localStorage.
    const compact={attendance:Array.isArray(ops?.attendance)?ops.attendance:[],fees:Array.isArray(ops?.fees)?ops.fees.map(p=>{
      const copy={...p};
      const student=state.db?.classes?.[String(copy.class)]?.students?.find(s=>s.id===copy.studentId);
      if(student?.photo) delete copy.photo;
      return copy;
    }):[]};
    localStorage.setItem(OPS_KEY,JSON.stringify(compact));
    if(message)toast(message);
    return true;
  }catch(_){
    // One recovery attempt: remove redundant cached receipt photos from old records.
    try{
      const compact={attendance:Array.isArray(ops?.attendance)?ops.attendance:[],fees:Array.isArray(ops?.fees)?ops.fees.map(p=>{const copy={...p};delete copy.photo;return copy}):[]};
      localStorage.setItem(OPS_KEY,JSON.stringify(compact));
      if(message)toast(message);
      return true;
    }catch(__){toast('Could not save school data in this browser.');return false}
  }
}
...
function printAttendance(cls,date,records,teacher){
  ...
  // school class remains on ONE A4 page even with many students.
  const groups=students.length>240?6:students.length>180?5:students.length>120?4:students.length>90?3:(students.length>45?2:1);
  ...
  $('printArea').innerHTML=`<div class="print-sheet attendance-a4 attendance-${density}" style="--att-cols:${groups};--att-rows:${maxRows}">
    <div class="a4-soft-border">
      <div class="attendance-print-header">
        <img src="assets/image.svg.png" alt="Alhera English Medium School" class="attendance-logo">
        <div class="attendance-brand">
          <div class="attendance-brand-name">Alhera English Medium School</div>
          <div class="attendance-brand-sub">ENGLISH MEDIUM SCHOOL · SCHOOL ATTENDANCE PRO</div>
        </div>
  ...
  closeModal();
  setTimeout(()=>preparePrintAreaAndPrint(),40);
}
...
function printFeeReceipt(payment,teacher){
  ...
  payment.photo=st.photo||payment.photo||'';
      ...
      <div class="fee-receipt-card">
        <div class="fee-receipt-top">
          <div class="fee-brand-block">
            <div class="fee-brand-row"><img src="assets/image.svg.png" alt="Alhera English Medium School"><div><div class="fee-brand-big">Alhera English Medium School</div><div class="fee-brand-small">ENGLISH MEDIUM SCHOOL · FEE MANAGEMENT</div></div></div>
            <div class="fee-address-line">Jamudalak</div>
          </div>
        ...
        <div class="fee-footer-strip"><span>Alhera English Medium School</span><span>Computer Generated Receipt · Keep this receipt for your records</span></div>
      </div>
    </div>
  </div>`;
  closeModal();
...
function printFeeSummary(payments,teacher){const rows=payments.map((p,i)=>`<tr><td>${i+1}</td><td>${escapeHTML(p.studentName)}</td><td>${formatDate(p.date)} · ${escapeHTML(p.months)}</td><td>₹ ${Number(p.amount||0).toFixed(2)}</td></tr>`).join('');const total=payments.reduce((n,p)=>n+Number(p.amount||0),0);$('printArea').innerHTML=`<div class="print-sheet"><div class="print-border ops-print"><div class="print-header"><img src="assets/image.svg.png" alt="Alhera English Medium School"><div><div class="print-title">Alhera English Medium School</div><div class="print-subtitle">FEE COLLECTION SUMMARY</div></div></div><div class="print-heading">CLASS ${escapeHTML(state.selectedClass)}</div><table class="print-table"><thead><tr><th>Sl. No.</th><th>Name</th><th>Date / Month(s)</th><th>Amount</th></tr></thead><tbody>${rows||'<tr><td colspan="4">No payments</td></tr>'}</tbody></table><div class="fee-summary-total">Total Collected: <b>₹ ${total.toFixed(2)}</b></div><div class="print-signature-row"><div class="signature-box"><strong>${escapeHTML(teacher)}</strong><div class="signature-line"></div><small>Authorized Signature</small></div></div></div></div>`;closeModal();setTimeout(()=>preparePrintAreaAndPrint(),40)}
function openIdCardGenerator(){
  const students=classData().students;
  if(!students.length){toast('Add a student first.');return}
  openModal(`<div class="modal-head"><div><span class="eyebrow">ID CARD STUDIO</span><h3>Professional Student ID Card</h3><p class="muted">Sample-style card with editable Student ID.</p></div><button class="close-modal" type="button" data-modal-close>×</button></div><div class="form-grid two"><label class="field"><span>Select Student</span><select id="idCardStudent">${students.map(st=>`<option value="${escapeHTML(st.id)}">${escapeHTML(st.name)}</option>`).join('')}</select></label><label class="field"><span>Student ID</span><input id="idCardStudentCode" type="text" maxlength="40" autocomplete="off" placeholder="Enter Student ID"></label></div><div id="idCardPreview" class="id-card-preview"></div><div class="modal-actions"><button class="btn outline" type="button" data-modal-close>Cancel</button><button class="btn dark" id="printIdCard" type="button">Print ID Card</button></div>`,card=>{
    const sel=card.querySelector('#idCardStudent'),code=card.querySelector('#idCardStudentCode'),preview=card.querySelector('#idCardPreview');
    const draw=(resetCode=false)=>{const st=students.find(x=>x.id===sel.value)||students[0];if(resetCode)code.value=st.studentCode||'';preview.innerHTML=`<div class="id-sample-card-preview"><div class="id-preview-logo"><img src="assets/image.svg.png" alt="Alhera English Medium School"></div><div class="id-preview-photo">${st.photo?`<img src="${escapeHTML(st.photo)}" alt="Student Photo">`:`<div>${escapeHTML(st.name.charAt(0).toUpperCase())}</div>`}</div><div class="id-preview-name">${escapeHTML(st.name)}</div><div class="id-preview-class">CLASS — ${escapeHTML(state.selectedClass)}</div><div class="id-preview-details"><div><b>Student ID</b><span>${escapeHTML(String(code.value.trim()||String(st.id).slice(-8).toUpperCase()))}</span></div><div><b>Parents Name</b><span>${escapeHTML(st.parentName||'—')}</span></div><div><b>Mobile Number</b><span>${escapeHTML(st.mobile||'—')}</span></div><div><b>Address</b><span>${escapeHTML(st.address||'—')}</span></div></div><div class="id-preview-footer">Alhera English Medium School · ENGLISH MEDIUM SCHOOL</div></div>`};
    sel.onchange=()=>draw(true);code.oninput=()=>draw(false);draw(true);
    card.querySelector('#printIdCard').onclick=()=>{const st=students.find(x=>x.id===sel.value);if(!st)return;const value=code.value.trim();if(!value){toast('Enter Student ID first.');code.focus();return}const duplicate=students.some(x=>x.id!==st.id&&String(x.studentCode||'').trim().toLowerCase()===value.toLowerCase());if(duplicate){toast('That Student ID already exists in this class.');code.focus();return}st.studentCode=value;if(!persist())return;openIdCardSignature(st)};
  })
}
...
function printIdCard(st,teacher){
  const sid=String(st.studentCode||String(st.id).slice(-8).toUpperCase()).trim();
    ...
    <div class="id-a4-page id-sample-a4-page">
      <div class="id-sample-paper-border">
        <div class="id-sample-card">
          ...
          <div class="id-sample-logo"><img src="assets/image.svg.png" alt="Alhera English Medium School"></div>
          ...
          <div class="id-sample-footer"><strong>Alhera English Medium School</strong><span>ENGLISH MEDIUM SCHOOL</span></div>
        </div>
      </div>
    </div>
  </div>`;
...
function preparePrintAreaAndPrint(){
  const area=$('printArea');
  if(!area)return;
  // Print directly after the print DOM is mounted. This keeps Android/browser
  // print activation reliable while the browser handles image/font painting.
  requestAnimationFrame(()=>requestAnimationFrame(()=>window.print()));
}

window.addEventListener('afterprint',()=>{const area=$('printArea');if(area){area.innerHTML='';}});

// Keep the main page scrollable on Android Chrome/PWA even after a modal has been closed.
...
window.addEventListener('DOMContentLoaded',()=>setTimeout(restorePageScroll,0));
window.addEventListener('pageshow',restorePageScroll);
window.addEventListener('resize',restorePageScroll);
...
async function printResult(exam=currentExam()){
  ...
  await preparePrintAreaAndPrint();
}
...
function buildBackupPayload(){return{app:'Alhera English Medium School — Student Result Manager Pro',backupVersion:BACKUP_VERSION,createdAt:new Date().toISOString(),data:state.db,operations:loadOps()}}
...
function exportBackup(){downloadBlob(backupBlob(),`alhera-school-backup-${todayISO()}.json`);toast('Backup downloaded.')}
async function shareBackup(){const blob=backupBlob(),file=new File([blob],`alhera-school-backup-${todayISO()}.json`,{type:'application/json'});try{if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:'Alhera English Medium School Result Backup',text:'Student Result Manager backup',files:[file]});toast('Backup shared successfully.')}else{exportBackup();toast('Sharing is not supported here. Backup downloaded instead.')}}catch(e){if(e?.name!=='AbortError')exportBackup()}}
...
function openDataHealthModal(){
  ...
  openModal(`<div class="modal-head"><div><span class="eyebrow">DATA HEALTH</span><h3>${ok?'Everything looks healthy':'Attention required'}</h3><p class="muted">A complete consistency check of stored result data.</p></div><button class="close-modal" type="button" data-modal-close>×</button></div><div class="health-box ${ok?'ok':'bad'}"><strong>${ok?'No data problems found':'Problems detected: '+problems}</strong><p>${ok?'Classes, students, exams, statuses and marks are consistent.':'Review the details below before relying on a backup.'}</p></div><div class="health-grid"><div class="health-box"><strong>Invalid marks</strong><p>${invalid}</p></div><div class="health-box"><strong>Duplicate students</strong><p>${duplicates}</p></div><div class="health-box"><strong>Orphan marks</strong><p>${orphanMarks}</p></div><div class="health-box"><strong>Orphan statuses</strong><p>${orphanStatuses}</p></div><div class="health-box"><strong>Invalid statuses</strong><p>${invalidStatus}</p></div><div class="health-box"><strong>Storage summary</strong><p>${CLASSES.length} classes · ${totalStudents} students · ${totalExams} exams · ${totalMarks} marks</p></div></div><div class="modal-actions"><button class="btn outline" type="button" data-modal-close>Close</button></div>`);
}
...
function openGlobalSearchModal(){
  openModal(`<div class="modal-head"><div><span class="eyebrow">GLOBAL SEARCH</span><h3>Find a Student</h3><p class="muted">Search across Class 4 to Class 8.</p></div><button class="close-modal" type="button" data-modal-close>×</button></div><label class="search global-search"><span>⌕</span><input id="globalSearchInput" type="search" placeholder="Student name" autocomplete="off"></label><div id="globalSearchResults" class="global-results"><div class="empty"><p>Type a student name to search.</p></div></div>`,card=>{
